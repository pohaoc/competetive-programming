...
// From https://github.com/ecnerwala/icpc-book/blob/master/content/numerical/fft.cpp
namespace fft {
...
vector<long long> multiply_int(const vi& a, const vi& b) {
    int s = sz(a) + sz(b) - 1;
    if (s <= 0) return {};
    int L = s > 1 ? 32 - __builtin_clz(s-1) : 0, n = 1 << L;
    if (sz(fa) < n) fa.resize(n);
...
int main() {
    ...
    vector<double> a{1.5,2,3};
    vector<double> b{1.0,2,3};
